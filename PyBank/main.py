...
out_file = os.path.join("Analysis","budget_analysis.txt")
...
output = (
    ...
    f"Average Change: ${round(totalchange/(len(tallytotal)),2)}\n"
    f"Greatest Increase in Profits: {increasedate} $({round(mtallytotal,2)})\n"
    f"Greatest Decrease in Profits: {decreasedate} $({round(mintallytotal,2)})\n"
)
print(output)
